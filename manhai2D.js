...
(function (global, factory) {
	...
	function Camera(xView, yView, width, height)
	{
		...
		else
		{
			...
			this.column = 0;
			this.row = 0;
			this.worldSize = new rectangleObject(0,0,0,0);
			this.worldEqualTilemap = false;
			this.tileSize = 32;
		}
	}
	...
	Object.assign(Camera.prototype, {
		...
		moveX: function(x)
		{
			if(this.worldEqualTilemap == true)
			{
				if(((this.xView + (-x)) <= this.worldSize.x) && ((this.xView + (-x)) >= (-(this.worldSize.right - this.width))))
				{
					this.xView += (-x);
				}
				else if((this.xView + (-x)) > this.worldSize.x)
				{
					this.xView = this.worldSize.x;
				}
				else if((this.xView + (-x)) < -(this.worldSize.right - this.width))
				{
					this.xView = -(this.worldSize.right - this.width);
				}
			}
			else
			{
		...
		moveY: function(y)
		{
			if(this.worldEqualTilemap == true)
			{
				if(((this.yView + (-y)) <= this.worldSize.y) && ((this.yView + (-y)) >= (-(this.worldSize.bottom - this.height))))
				{
					this.yView += (-y);
				}
				else if((this.yView + (-y)) > this.worldSize.y)
				{
					this.yView = this.worldSize.y;
				}
				else if((this.yView + (-y)) < -(this.worldSize.bottom - this.height))
				{
					this.yView = -(this.worldSize.bottom - this.height);
				}
			}
			else
			{
		...
		setTileMap: function(map, column, row)
		{
			...
			if(typeof(column) == "number" && typeof(row) == "number")
			{
				...
				this.worldSize.set(0,0, this.column * this.tileSize, this.row * this.tileSize);
			}
			else if(typeof(column) != "number")
			{
				console.log("The argument 'column' isn't a number.");
			}
			else if(typeof(row) != "number")
			{
				console.log("The argument 'row' isn't a number.");
			}
		},
		...
		setWorldEqualTilemap: function(bool)
		{
			if(bool == true || bool == false)
			{
				this.worldEqualTilemap = bool;
			}
			else
			{
				console.log("The argument for 'setWorldEqualTilemap' isn't a boolean.");
			}
		},
	...
	Object.assign(Renderer.prototype, {
		...
		start:function(method)
		{
			...
			if(this.scene.camera != null)
			{
				...
				if(this.scene.camera.tileMap.length > 0)
				{
					for(let c = 0; c < this.scene.camera.column; c++)
					{
						for(let r = 0; r < this.scene.camera.row; r++)
						{
							let ind = isItemIn2DArray(this.scene.camera.mapDecoder, this.scene.camera.getTile(c,r), 0);
							
							if(typeof(ind) == "number")
							{
								this.ctx.drawImage(this.scene.camera.mapDecoder[ind][1].material, c * this.scene.camera.tileSize, r * this.scene.camera.tileSize, this.scene.camera.tileSize, this.scene.camera.tileSize);
								
								this.scene.camera.mapDecoder[ind][1].getX(c * this.scene.camera.tileSize);
								this.scene.camera.mapDecoder[ind][1].getY(r * this.scene.camera.tileSize);
								this.scene.camera.mapDecoder[ind][1].getWidth(this.scene.camera.tileSize);
								this.scene.camera.mapDecoder[ind][1].getHeight(this.scene.camera.tileSize);	
							}
						}
					}
				}
